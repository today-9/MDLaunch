...
| タグ | チップ式ピッカーで既存タグから選択。サイドバーのタグを右クリックで 11 色のカラーパレット |
| 検索 | `Ctrl + K` でタイトル+全文のインクリメンタル検索。タグチップで絞り込み(複数選択で AND)、テキストとの併用も可 |
...
- タグの色はサイドバーのタグを右クリックで設定(`vault/.mdlaunch/tags.json` に保存)
